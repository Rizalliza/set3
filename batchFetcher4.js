...
function toBN(val) {
    if (val === undefined || val === null) return new BN(0);
...
class RPCConnectionManager {
    ...
    getWorkingRpcUrls() {
        const urls = [];
        if (process.env.RPC_URL2) urls.push(process.env.RPC_URL2);
        if (process.env.RPC_URL1) urls.push(process.env.RPC_URL1);
            ...
            for (let i = 0; i < chunks.length; i++) {
                ...
                try {
                    const accounts = await this.connection.getMultipleAccountsInfo(
                        chunk.map(p => new PublicKey(p.address)),
                        'confirmed'
                    );
...
class ReserveQuoteEngine {
    ...
    getQuoteFixed(pool, inputMint, outputMint, amountIn) {
        try {
            ...
            const isAtoB = inputMint === pool.mintA && outputMint === pool.mintB;
            const isBtoA = inputMint === pool.mintB && outputMint === pool.mintA;
            ...
            const feeBps = reserves.feeBps || pool.feeBps || 25;
            const feeNumerator = new BN(feeBps);
            ...
            if (!reserveIn || !reserveOut || reserveIn.amount.isZero() || reserveOut.amount.isZero()) {
                return { amountOut: new BN(0), feeBps };
            }

            // Constant product formula: amountOut = (amountIn * 997 * reserveOut) / (reserveIn * 1000 + amountIn * 997)
            const amountInWithFee = amount.mul(feeDenominator.sub(feeNumerator));
            const numerator = amountInWithFee.mul(reserveOut.amount);
            const denominator = reserveIn.amount.mul(feeDenominator).add(amountInWithFee);
            ...
            return {
                amountOut: amountOut.gt(reserveOut.amount) ? reserveOut.amount : amountOut,
                feeBps
            };
...
function calculatePriceDiffs(results, testAmountLamports) {
    ...
    Object.entries(byPair).forEach(([pair, items]) => {
        items.sort((a, b) => {
            if (a.quote.amountOut.gt(b.quote.amountOut)) return -1;
            if (a.quote.amountOut.lt(b.quote.amountOut)) return 1;
            return 0;
        });
        const best = items[0];
...
async function exportAnalysis(leg1Data, leg2Data, leg3Data, testAmountSOL) {
    ...
    function createSheet(name, data, columns) {
        ...
        data.forEach((row, idx) => {
            ...
            const diffCell = r.getCell('priceDiffBps');

            if (row.isBestInPair) {
                diffCell.fill = bestFill;
                r.getCell('pair').fill = bestFill;
            } else if (row.priceDiffBps < 50) {
...
function generateTriangleCandidates(leg1, leg2, leg3) {
    ...
    for (const l1 of bestLeg1) {
        ...
        for (const l2 of bestLeg2) {
            if (l2.inputMint !== tokenX) continue;
            const tokenY = l2.outputMint;

            for (const l3 of bestLeg3) {
                if (l3.inputMint !== tokenY || l3.outputMint !== SOL) continue;
...
//. node src/arbitrage/batchFetcher4.js pool_list_enriched.json --use-analysis